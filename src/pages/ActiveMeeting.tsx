...
export function ActiveMeeting() {
  ...
  const handleAddTodo = () => {
    if (newTodo.trim()) {
      setTodos([...todos, { id: Date.now().toString(), text: newTodo, checked: false, status: '未开始', assignee: '待分配', dueDate: new Date().toISOString().split('T')[0] }]);
      setNewTodo('');
    }
  };
  ...
  const handleGenerateSummary = async () => {
    ...
    try {
      ...
      alert(error instanceof Error ? error.message : '总结生成失败');
    } finally {
          ...
          <div className="bg-slate-900 rounded-2xl p-8 w-full max-w-lg shadow-2xl border border-slate-700 text-slate-100">
            ...
            {isGeneratingSummary && (
              <div className="flex flex-col items-center gap-4 py-8">
                ...
                <p className="text-slate-400">正在生成总结...</p>
              </div>
            )}
            {summaryData && (
